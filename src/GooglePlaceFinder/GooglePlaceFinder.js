...
class GooglePlaceFinder extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      loading: false,
  ...
  init = () => {
    if (!window.google) {
      throw new Error(
        "[flexmoney-google-places-demo] Google Maps JavaScript API library must be loaded"
      );
    }

    if (!window.google.maps.places) {
      throw new Error(
        "[flexmoney-google-places-demo] Google Maps Places library must be loaded. Please add `libraries=places` to the src URL."
      );
    }
  ...
  getInputProps = (options = {}) => {
    if (options.hasOwnProperty("value")) {
      throw new Error(
        "[flexmoney-google-places-demo] getInputProps does not accept `value`. Use `value` prop instead"
      );
    }

    if (options.hasOwnProperty("onChange")) {
      throw new Error(
        "[flexmoney-google-places-demo] getInputProps does not accept `onChange`. Use `onChange` prop instead"
      );
    }
...
GooglePlaceFinder.defaultProps = {
  searchOptions: {},
  debounce: 300,
